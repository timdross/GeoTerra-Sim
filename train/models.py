...
class DEMRangeHead(nn.Module):
    """
    Predicts dem_range from the bottleneck DEM latent.
    Attaches to the UNet's mid_block output — no extra UNet channels needed.
    """
    def __init__(self, latent_channels: int = 4, hidden_dim: int = 256):
        super().__init__()
    ...
    def forward(self, dem_latent: torch.Tensor) -> torch.Tensor:
        """
        Args:
            dem_latent: (B, 4, 96, 96) — the DEM latent from the UNet output
        Returns:
            dem_range_pred: (B, 1) — predicted elevation range in normalized space
        """
        pooled = self.pool(dem_latent)  # (B, 4, 1, 1)
...
# ==========================================
# 1. DiffusionSat Metadata Encoder
# ==========================================
class MetadataEncoder(nn.Module):
    """
    Encodes scalar metadata (NLCD + height range) into a continuous 
    embedding that matches the timestep dimension, as detailed in DiffusionSat.
    """
    def __init__(self, num_metadata_features: int, time_embed_dim: int):
        super().__init__()
        self.mlp = nn.Sequential(
            nn.Linear(num_metadata_features, time_embed_dim // 2),
            nn.SiLU(),
            nn.Linear(time_embed_dim // 2, time_embed_dim)
        )

    def forward(self, metadata: torch.Tensor, drop_prob: float = 0.1):
        if self.training and drop_prob > 0:
            mask = torch.rand(metadata.shape[0], 1, device=metadata.device) > drop_prob
            metadata = metadata * mask.float()
